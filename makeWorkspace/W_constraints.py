...
def cmodel(cid,nam,_f,_fOut, out_ws, diag, year):
  ...
  add_variation(WScales_e, felectronid, "{CHANNEL}_{YEAR}_1e_id_up".format(**filler), "wen_weights_%s_CMS_eff%s_ele_Up"%(cid, year), _fOut, invert=True)
  add_variation(WScales_e, felectronid, "{CHANNEL}_{YEAR}_1e_id_dn".format(**filler), "wen_weights_%s_CMS_eff%s_ele_Down"%(cid, year), _fOut, invert=True)
  CRs[1].add_nuisance_shape("CMS_eff{YEAR}_ele".format(**filler),_fOut)
  add_variation(WScales_e, felectronid, "{CHANNEL}_{YEAR}_1e_reco_up".format(**filler), "wen_weights_%s_CMS_eff%s_ele_reco_Up"%(cid, year), _fOut, invert=True)
  add_variation(WScales_e, felectronid, "{CHANNEL}_{YEAR}_1e_reco_dn".format(**filler), "wen_weights_%s_CMS_eff%s_ele_reco_Down"%(cid, year), _fOut, invert=True)
