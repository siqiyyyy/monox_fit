...
#     for tag in [""]:
#         fitdiag_file = 'diagnostics/fitDiagnostics_monojet_{year}{tag}.root'.format(year=year,tag=tag)
#         diffnuis_file = 'diagnostics/diffnuisances_monojet_{year}{tag}.root'.format(year=year,tag=tag)
...
for tag in [""]:
    ...
    diffnuis_file = 'diagnostics/diffnuisances_monojet_combined{tag}.root'.format(tag=tag)
    plot_nuis(diffnuis_file, outdir)

    for year in [2017,2018]:
        ws_file = "root/ws_monojet.root".format(year=year)
        fitdiag_file = 'diagnostics/fitDiagnostics_monojet_combined{tag}.root'.format(year=year,tag=tag)
        category='monojet_{year}'.format(year=year)
